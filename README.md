...
```
┌─────────────┐
│  Publisher  │  Cria OrderCreated event
└──────┬──────┘
...
│  (In-Memory Bus) │
└──────┬───────────┘
       │ Passa mensagem para Router
       ▼
┌──────────────────────────────────────────────────────┐
│                 Router Watermill                      │
├──────────────────────────────────────────────────────┤
│  ┌────────────────────────────────────────────────┐  │
│  │  1️⃣ Recover Middleware                         │  │
│  │     └─ Captura panics, evita crash            │  │
│  ├────────────────────────────────────────────────┤  │
│  │  2️⃣ EnsureCorrelationID (Custom)              │  │
│  │     └─ Gera correlation_id se não existir    │  │
│  ├────────────────────────────────────────────────┤  │
│  │  3️⃣ CorrelationID (Watermill)                 │  │
│  │     └─ Propaga correlation_id no pipeline    │  │
│  ├────────────────────────────────────────────────┤  │
│  │  4️⃣ Retry Middleware                          │  │
│  │     └─ MaxRetries: 2 | InitialInterval: 200ms│  │
│  ├────────────────────────────────────────────────┤  │
│  │  5️⃣ Poison Queue Handler (DLQ)                │  │
│  │     └─ Encaminha falhas para DLQ              │  │
│  └────────────────────────────────────────────────┘  │
└──────┬───────────────────────────────────────────────┘
       │ Passa para Handler
       ▼
┌──────────────────┐
│  OrderHandler    │  Deserializa + Log + Lê correlation_id
└──────┬───────────┘
       │ Chama usecase
       ▼
┌──────────────────┐
│  ProcessOrder    │  Valida regras de negócio
│   (Usecase)      │
└──────┬───────────┘
       │
       ├─ ✅ Sucesso → Mensagem ACK
       │
       ├─ ⚠️  Erro de Negócio → Não vai para DLQ
       │   (ErrSaldoInsuficiente, ErrEstoqueIndisponivel)
       │
       └─ 💥 Erro Técnico → DLQ (order-created-dlq)
          (Exceção não tratada, timeout, etc)
```
